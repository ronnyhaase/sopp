 ...
 */
// TODO: One full item for every illegal option
// TODO: Default type is Boolean
// TODO: Implement sopp(...).printHelp()
// TODO: Implement files only delimiter '--' if desired
// TODO: Every option must have: ... (?) -> noConflict()
// FIX: Passing no 'short' or no 'long' in an option shouldn't fail
...
module.exports = exports = function sopp(settings, argv, slicepos) {
	...
	(function noConflict(opts) {
		...
		for (var i = 0, len = opts.length; i < len; i++) {
			...
			if ( opts[i].hasOwnProperty('short') ) {
	...
	function addIllegalShort(s) {
		if ( parsed.illegal_.indexOf('-' + s) === -1)
			parsed.illegal_.push('-'+ s)
	}

	function addIllegalLong(l) {
		if ( parsed.illegal_.indexOf(l) === -1)
			parsed.illegal_.push(l)
	}
	...
	for (var i = 0; i < argc; i++) {
		...
		if ( isShortOpt(arg) ) {
			...
			//
			var
				// Position of equal-character or EOS
